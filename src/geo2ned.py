...
class NED:
    def __init__(self, lat, lon, height):
        ...
        # Save NED origin
        self.init_lat = np.radians(lat)
        self.init_lon = np.radians(lon)
        self.init_h = height
    ...
    def geodetic2ecef(self, coord):
        ...
        lat = np.radians(coord[0])
        lon = np.radians(coord[1])
        
        xi = np.sqrt(1 - self.esq * np.sin(lat)**2)
        x = (self.a / xi + coord[2]) * np.cos(lat) * np.cos(lon)
        y = (self.a / xi + coord[2]) * np.cos(lat) * np.sin(lon)
        z = (self.a / xi * (1 - self.esq) + coord[2]) * np.sin(lat)
